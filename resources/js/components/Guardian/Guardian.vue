            ...
            <card>
                ...
                <form @submit.prevent="submitForm()">
                    ...
                    <button type="submit" class="btn btn-primary" :disabled="submit">
                        <span v-if="formType == 'create'">Create Guardian</span>
                        <span v-else-if="formType == 'update-account'">Update Account</span>
                        <span v-else>Update Guardian</span>
                    </button>
...
<script>
    ...
    export default {
        ...
        methods: {
            submitForm: debounce(function(){
                ...
                if(this.formType == "update"){
                    ...
                    .then(res => {
                        ...
                        this.getGuardians();
                        this.resetForm();
                        alert(`Guardian information has been updated.`);
                    })
                    .catch(err => {
                        this.submit = false;
                    ...
                    axios.put(route('guardians.update', this.formData.userable_id), this.formData)
                    .then(res => {
                        this.submit = false;
                        this.getGuardians();
                        this.resetForm();
                        alert(`Guardian account has been updated.`);
                    })
                    .catch(err => {
                        this.submit = false;
                        this.formErrors = err.response.data.errors
            ...
            editAccount(guardian){
                if(guardian.user){
                    this.formData = cloneDeep(guardian.user);
                }
            ...
            setAccountDetails(e){
                let guardian_id_number = e.target.value;
                this.formData = {
                    ...this.formData,
                    username: guardian_id_number,
                    password: guardian_id_number,
                }
            }
            // as
        },
